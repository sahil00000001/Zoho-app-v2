...
const STATUS_STYLE: Record<string, string> = {
  ...
  CANCELLED: "bg-slate-100 text-gray-500 border-slate-200",
};
  ...
  return (
    <div className="p-6 space-y-5 animate-fade-in">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-[22px] font-bold text-slate-900 leading-none">Leave Approvals</h1>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white border border-slate-200 rounded-2xl p-5 flex items-center gap-4">
          ...
          <div>
            <div className="text-2xl font-bold text-slate-900 leading-none">{counts.PENDING}</div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 flex items-center gap-4">
          ...
          <div>
            <div className="text-2xl font-bold text-slate-900 leading-none">{counts.APPROVED}</div>
            <div className="text-xs text-slate-500 mt-0.5 font-medium">Approved</div>
          </div>
        </div>
        <div className="bg-white border border-slate-200 rounded-2xl p-5 flex items-center gap-4">
          ...
          <div>
            <div className="text-2xl font-bold text-slate-900 leading-none">{counts.REJECTED}</div>
            <div className="text-xs text-slate-500 mt-0.5 font-medium">Rejected</div>
          </div>
        </div>
      </div>
          ...
          {[1, 2, 3].map((i) => (
            <div key={i} className="bg-white border border-slate-200 rounded-2xl p-5 animate-pulse">
              <div className="flex items-center gap-4">
                ...
                <div className="h-8 bg-slate-100 rounded-lg w-20 shrink-0" />
              </div>
            </div>
          ))}
        </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2 flex-wrap">
                        <div>
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {leave.user.department?.name && (
                              <span className="text-[11px] bg-slate-100 text-gray-500 px-2 py-0.5 rounded-full">
                                {leave.user.department.name}
                              </span>
                            )}
                          </div>
                        ...
                        <span className={`text-[11px] font-semibold px-2.5 py-1 rounded-full border shrink-0 ${STATUS_STYLE[leave.status] ?? "bg-slate-100 text-gray-500"}`}>
                          {leave.status}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-2 flex-wrap mt-2.5">
                        ...
                        <span className="text-[11px] font-bold text-gray-700 bg-slate-100 px-2 py-0.5 rounded-full">
                          {days} day{days !== 1 ? "s" : ""}
                        </span>
                      </div>
                  ...
                  <div className="border-t border-red-100 bg-red-50/60 px-5 py-4">
                    ...
                    <div className="flex gap-2 mt-2.5">
                      ...
                      <button
                        ...
                        disabled={rejecting}
                        className="text-xs font-semibold text-gray-500 hover:text-gray-700 px-4 py-2 rounded-lg bg-white border border-slate-200 hover:bg-slate-50 transition-colors"
                      >
                        Cancel
                      </button>
                    </div>
